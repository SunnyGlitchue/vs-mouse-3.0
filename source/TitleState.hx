...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg = new FlxSprite(-80).loadGraphic(Paths.image('MENU'));
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				transitioning = true;
				FlxTween.tween(logoBl, {y: logoBl.y + 900}, 0.9, {ease: FlxEase.cubeInOut});
				FlxTween.tween(bg, {x: bg.x + 1400}, 1.2, {ease: FlxEase.cubeInOut});
