...
class PlayState extends MusicBeatState
{
	...
	//DISK3
	var sky:BGSprite;
	var neighborhood:BGSprite;
	var street:BGSprite;

	//EVIL DISK 3
	var skye:BGSprite;
	var neighborhoode:BGSprite;
	var streete:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'clubhouse-day': //Disk1
				var bg:BGSprite = new BGSprite('clubhouse/ClubHouse_Day', -600, -500, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 1.7));
				add(bg);

				case 'disney-land': //Disk2
				var bg:BGSprite = new BGSprite('labrat bg/disney', -600, -600, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 1.7));
				add(bg);

				case 'suicide': //Disk3
				var sky:BGSprite = new BGSprite('alley/sky', -600, -600, 0.9, 0.9);
				sky.setGraphicSize(Std.int(sky.width * 1.5));
				add(sky);

				var neighborhood:BGSprite = new BGSprite('alley/neighborhood', -600, -600, 0.9, 0.9);
				neighborhood.setGraphicSize(Std.int(neighborhood.width * 1.5));
				add(neighborhood);

				var street:BGSprite = new BGSprite('alley/floor', -600, -600, 0.9, 0.9);
				street.setGraphicSize(Std.int(street.width * 1.5));
				add(street);

				switch (SONG.song.toLowerCase()) {
					case 'suicide' | 'happy-ride' | 'insanity-psychosis':
					remove(neighborhood);
					remove(sky);
					remove(street);

					var skye:BGSprite = new BGSprite('alley/sky2', -600, -600, 0.9, 0.9);
					skye.setGraphicSize(Std.int(skye.width * 1.5));
					add(skye);
	
					var neighborhoode:BGSprite = new BGSprite('alley/neighborhood2', -600, -600, 0.9, 0.9);
					neighborhoode.setGraphicSize(Std.int(neighborhoode.width * 1.5));
					add(neighborhoode);
	
					var streete:BGSprite = new BGSprite('alley/floor2', -600, -600, 0.9, 0.9);
					streete.setGraphicSize(Std.int(streete.width * 1.5));
					add(streete);
				}

				var shadow:BGSprite = new BGSprite('alley/shadow', -600, -600, 0.9, 0.9);
				shadow.setGraphicSize(Std.int(shadow.width * 1.5));
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'repentance':
                    startVideo('Disk 1 Scene 2');
				
				case 'revenge':
                    startVideo('Disk 1 Scene 3');

				case 'interference':
                    startVideo('Disk 2 Scene 1');
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Black Tween':
				blacky = new FlxSprite(200, -300).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
				blacky.scrollFactor.set();
				blacky.alpha = 1;
				add(blacky);

				FlxTween.tween(blacky, {alpha: 0}, 1, {ease: FlxEase.circOut});
